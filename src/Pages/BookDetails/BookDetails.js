...
const BookDetails = () => {
    ...
    const fetchBookDetails = async () => {
      try {
        ...
        if (response?.status === 200 && response?.data?.items?.length > 0) {
          const { title, authors, saleInfo, description, imageLinks } = response.data.items[0].volumeInfo;

          setBook({
            title: title || "Chain of Gold: The Last Hours #1",
            authors: authors ? authors.join(", ") : "No Authors",
            price: saleInfo && saleInfo.retailPrice ? `$${saleInfo.retailPrice.amount}` : "12.45$",
            description: description || "No Description Available",
            imageUrl: imageLinks ? imageLinks.thumbnail : "../images/book-slider/prin-img.png",
          });
        } else {
  ...
  return (
    <>
      ...
      <BooksCarousels carusalTitle={"Collection"} />
    </>
  );
};
